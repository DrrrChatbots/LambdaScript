...
makeTokenParser (LanguageDef languageDef)
  ...
  where
    ...
    keyLiteral :: ParserT String m String
    keyLiteral = lexeme (go <?> "literal key")
      where
        go :: ParserT String m String
        go = do
            maybeChars <- (List.many keyChar)
    ...
    keyChar :: ParserT String m (Maybe Char)
    keyChar = (Just <$> keyLetter)
             -- <|> keyEscape
             <?> "key character"
